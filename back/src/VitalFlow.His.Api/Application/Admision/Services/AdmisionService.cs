...
public sealed class AdmisionService(
    ...
    IHttpContextAccessor httpContextAccessor) : IAdmisionService
{
    ...
    private const string MotivoClearingAusente = "CLEARING_AUSENTE";
    private const string MotivoClearingNoAtendido = "CLEARING_NO_ATENDIDO";
    private const string MotivoClearingAtendido = "CLEARING_ATENDIDO";
    private const string MotivoClearingNoAdmitido = "CLEARING_NO_ADMITIDO";
    private const string ModoAutomatico = "AUTOMATICO";
    ...
    private AdmisionScopeContext BuildScope()
    {
        ...
        if (user is null)
            return new AdmisionScopeContext(false, false, null, null, null);
    ...
    public SelectoresAdmisionResponse GetSelectores()
    {
    ...
    public EncuentroAdmisionResponse ObtenerEncuentro(string turnoId)
    {
        ...
        if (encuentro is not null)
        {
            return EncuentroRowToResponse(encuentro);
        }

        var turnoAdmision = admisionRepository.GetTurnoAdmision(turnoId)
            ?? throw new ArgumentException("No existe turno para el ID indicado.");

        var pacienteId = turnoAdmision.PacienteId
            ?? throw new ArgumentException("El turno no tiene un paciente asociado.");

        var encuentroId = admisionRepository.CrearEncuentroSiNoExiste(turnoId, pacienteId);
        encuentro = admisionRepository.GetEncuentroPorTurno(turnoId)
            ?? throw new ArgumentException("Error al crear el encuentro.");
        return EncuentroRowToResponse(encuentro);
    }
    ...
    public CerrarEncuentroResponse CerrarEncuentro(string turnoId, CerrarEncuentroRequest request)
    {
        ...
        if (string.Equals(estadoPacienteFinal, EstadoAtendido, StringComparison.OrdinalIgnoreCase))
        {
            var turnoGuid = Guid.TryParse(turnoId, out var parsed) ? parsed : Guid.Empty;
            if (turnoGuid != Guid.Empty)
            {
                var evolucion = historiaClinicaRepository.GetEvolucionAmbulatoriaByTurnoId(turnoGuid);
                if (evolucion is null)
                {
                    throw new ArgumentException("Debe registrar una evolucion antes de cerrar el encuentro.");
                }

                if (string.IsNullOrWhiteSpace(evolucion.Texto))
                {
                    throw new ArgumentException("La evolucion debe tener texto registrado.");
                }

                if (evolucion.ProblemasAsociados is null || evolucion.ProblemasAsociados.Count == 0)
                {
                    throw new ArgumentException("La evolucion debe tener al menos un problema asociado.");
                }
            }
        }
    ...
    public LimpiarEventosHuerfanosResponse LimpiarEventosHuerfanos(LimpiarEventosHuerfanosRequest? request = null)
    {
        var modo = string.Equals(request?.Modo, ModoManual, StringComparison.OrdinalIgnoreCase) ? ModoManual : ModoAutomatico;

        var programados = admisionRepository.GetTurnosEnEstados(new[] { EstadoProgramado });
        var salaEspera = admisionRepository.GetTurnosEnEstados(new[] { EstadoEnSalaEspera });
        var observacion = admisionRepository.GetTurnosEnEstados(new[] { EstadoEnObservacion });
        var pendientePago = admisionRepository.GetTurnosEnEstados(new[] { EstadoPendientePago });

        var procesados = new List<(string TurnoId, string EstadoAnterior, string EstadoNuevo)>();

            foreach (var turnoId in programados)
            {
                var rowActual = admisionRepository.GetTurnoAdmision(turnoId);
                if (rowActual is null) continue;

                admisionRepository.UpsertTurnoAdmision(new TurnoAdmisionRow(
                    turnoId, rowActual.PacienteId, rowActual.PacienteNombre, rowActual.Documento, rowActual.Financiador,
                    EstadoAusente, rowActual.EstadoTurno, MotivoClearingAusente, rowActual.LlegadaEn));
                procesados.Add((turnoId, EstadoProgramado, EstadoAusente));
            }

            foreach (var turnoId in salaEspera)
            {
                var rowActual = admisionRepository.GetTurnoAdmision(turnoId);
                if (rowActual is null) continue;
                ...
                admisionRepository.UpsertTurnoAdmision(new TurnoAdmisionRow(
                    turnoId, rowActual.PacienteId, rowActual.PacienteNombre, rowActual.Documento, rowActual.Financiador,
                    EstadoNoAtendido, rowActual.EstadoTurno, MotivoClearingNoAtendido, rowActual.LlegadaEn));
                procesados.Add((turnoId, EstadoEnSalaEspera, EstadoNoAtendido));
            }

            foreach (var turnoId in observacion)
            {
                var rowActual = admisionRepository.GetTurnoAdmision(turnoId);
                if (rowActual is null) continue;
                ...
                admisionRepository.UpsertTurnoAdmision(new TurnoAdmisionRow(
                    turnoId, rowActual.PacienteId, rowActual.PacienteNombre, rowActual.Documento, rowActual.Financiador,
                    EstadoAtendido, rowActual.EstadoTurno, MotivoClearingAtendido, rowActual.LlegadaEn));
                procesados.Add((turnoId, EstadoEnObservacion, EstadoAtendido));
            }

            foreach (var turnoId in pendientePago)
            {
                var rowActual = admisionRepository.GetTurnoAdmision(turnoId);
                if (rowActual is null) continue;
                ...
                admisionRepository.UpsertTurnoAdmision(new TurnoAdmisionRow(
                    turnoId, rowActual.PacienteId, rowActual.PacienteNombre, rowActual.Documento, rowActual.Financiador,
                    EstadoNoAdmitido, rowActual.EstadoTurno, MotivoClearingNoAdmitido, rowActual.LlegadaEn));
                procesados.Add((turnoId, EstadoPendientePago, EstadoNoAdmitido));
            }

            var progAusente = programados.Count;
            var salaNoAtend = salaEspera.Count;
            var obsAtend = observacion.Count;
            var pagoNoAdm = pendientePago.Count;
            var total = procesados.Count;

            var clearingId = admisionRepository.CreateClearingLog(
                request?.CentroId, modo, progAusente, salaNoAtend, obsAtend, pagoNoAdm, total);

            foreach (var (turnoId, estadoAnterior, estadoNuevo) in procesados)
            {
                admisionRepository.CreateClearingDetalle(clearingId, turnoId, estadoAnterior, estadoNuevo);
            }

        return new LimpiarEventosHuerfanosResponse(
                ClearingId: clearingId,
                EjecutadoEn: DateTimeOffset.UtcNow.ToLocalTime().ToString("dd/MM/yyyy HH:mm", CultureInfo.InvariantCulture),
                ProgramadosAAusente: progAusente,
                SalaEsperaANoAtendido: salaNoAtend,
                ObservacionAAtendido: obsAtend,
                PendientePagoANoAdmitido: pagoNoAdm,
