    ...
    IFhirService fhirService) : ControllerBase
{
        ...
        [FromQuery(Name = "_page")] int? page)
    {
        ...
        var result = fhirService.SearchSchedules(new SearchSchedulesRequest(
            actor, serviceType, location, active ?? true, effectiveCount, effectivePage, dateFilters));

        var baseUrl = $"{Request.Path}{Request.QueryString}";
        var entries = result.Entries.Select(s => new
        {
    ...
    public IActionResult GetSlotsR4(
        ...
        [FromQuery(Name = "_page")] int? page)
    {
        if (!FhirService.TryParseReferenceId(schedule, out var scheduleId))
        {
        ...
        [FromHeader(Name = "Correlation-Id")] string? correlationId)
    {
        ...
        var validated = fhirService.ValidateAndParseAppointmentBody(body, out var errorMessage, out var errorStatusCode);
        if (validated is null)
        {
            return StatusCode(errorStatusCode ?? 422, OperationOutcome("invalid", errorMessage ?? "Solicitud invalida."));
        }

        var slotReference = body.Slot?.FirstOrDefault()?.Reference;
        FhirService.TryParseReferenceId(slotReference, out var slotId);
        ...
        var patientId = FhirService.ExtractReferenceValue(patientReference ?? string.Empty);
        ...
        [FromQuery(Name = "_page")] int? page)
    {
        ...
        var patientId = FhirService.TryParseReferenceId(patient, out var patientGuid)
            ? patientGuid.ToString()
            : FhirService.ExtractReferenceValue(patient ?? string.Empty);
