...
public sealed class PostgresAuthRepository(string connectionString) : IAuthRepository
{
    ...
    public AuthRefreshTokenRow? GetRefreshTokenByHash(string tokenHash)
    {
        const string sql = """
            select id,
                   usuario_id,
    ...
    public IReadOnlyList<AuthRefreshTokenRow> GetActiveRefreshTokensByUser(Guid userId)
    {
        const string sql = """
            select id,
                   usuario_id,
        ...
        using var reader = cmd.ExecuteReader();
        var results = new List<AuthRefreshTokenRow>();
        while (reader.Read())
        {
            results.Add(new AuthRefreshTokenRow(
                reader.GetGuid(0),
                reader.GetGuid(1),
                reader.GetString(2),
                reader.GetFieldValue<DateTimeOffset>(3),
    ...
    public void RevokeAllUserRefreshTokens(Guid userId, DateTimeOffset revokedAt)
    {
        const string sql = """
