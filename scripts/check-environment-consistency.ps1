...
Assert-True (Test-ContainerRunning 'vitalflow_his_frontend') 'HIS frontend is running (vitalflow_his_frontend).'
...
$validPortalTargets = @(
    'http://host.docker.internal:3011/api',
    'http://his_backend:8080/api'
)
Assert-True ($validPortalTargets -contains $portalHisBase) "Portal uses a valid HIS_API_BASE_URL (actual: $portalHisBase)."

# HIS frontend proxy target
$frontendProxyLine = docker exec vitalflow_his_frontend sh -lc "cat /etc/nginx/conf.d/default.conf | grep proxy_pass" 2>$null
$frontendProxyLine = ($frontendProxyLine | Out-String).Trim()
$validFrontendProxy =
    ($frontendProxyLine -match 'proxy_pass\s+http://host\.docker\.internal:3011;') -or
    ($frontendProxyLine -match 'proxy_pass\s+http://his_backend:8080;') -or
    ($frontendProxyLine -match 'proxy_pass\s+\$his_upstream;')
Assert-True $validFrontendProxy "HIS frontend proxies /api to a valid HIS backend target (actual: $frontendProxyLine)."
