-- 043 - Homologacion practica HIS -> catalogo de facturacion (VitalFlow)
-- Objetivo: resolver que prestacion de catalogo debe usar el modulo de facturacion
-- al recibir un evento de admision desde HIS, segun practica + financiador/plan.
...
comment on table sch_admision.homologacion_practica_catalogo_facturacion
    is 'Reglas de homologacion entre practica de HIS y prestacion de catalogo para facturacion.';

comment on column sch_admision.homologacion_practica_catalogo_facturacion.practica_origen_codigo
    is 'Codigo de practica de HIS (sch_agenda.practica.codigo_clinico o codigo funcional equivalente).';

comment on column sch_admision.homologacion_practica_catalogo_facturacion.catalogo_codigo
    is 'Codigo de catalogo destino en facturacion. Default: VITALFLOW.';

comment on column sch_admision.homologacion_practica_catalogo_facturacion.prestacion_destino_codigo
    is 'Codigo de prestacion destino en catalogo de facturacion.';

comment on column sch_admision.eventos_facturacion_outbox.homologacion_encontrada
    is 'True cuando HIS pudo resolver homologacion practica->catalogo para el evento.';
