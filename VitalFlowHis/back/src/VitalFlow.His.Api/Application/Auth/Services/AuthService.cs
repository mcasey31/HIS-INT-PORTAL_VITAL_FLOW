    ...
    IJwtTokenService jwtTokenService) : IAuthService
{
    ...
    public AuthTokensResponse Refresh(RefreshRequest request, string? ip, string? userAgent)
    {
        ...
        var activeToken = authRepository.GetActiveRefreshToken(currentHash);

        if (activeToken is null || activeToken.ExpiresAt <= DateTimeOffset.UtcNow)
        {
            authRepository.InsertSesionLog(new CreateSesionLogRow(
                Guid.NewGuid(),
        ...
        var user = authRepository.GetUserById(activeToken.UsuarioId);
        ...
        authRepository.RevokeRefreshToken(activeToken.Id, DateTimeOffset.UtcNow, newRefreshHash);
