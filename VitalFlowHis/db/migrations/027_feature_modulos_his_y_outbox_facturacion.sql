-- 027 - Módulos HIS opcionales + Outbox de eventos para CONV-FACT
-- Módulo CONV-FACT es opcional; si no está activo, el outbox no recibe filas.

-- ── Módulos HIS (toggle por Estructura Interna) ─────────────────────────
...
values ('CONV_FACT', 'Convenios y Facturación', false)
on conflict (codigo) do nothing;

-- ── Outbox confiable: HIS → CONV-FACT ───────────────────────────────────
-- Un worker del módulo CONV-FACT lee PENDIENTE y crea el Episodio.
-- Si el módulo está desactivado, simplemente nadie lee este outbox.
